    ...
    async (req, res) => {
        ...
        try {
            ...
            if (comment && commentorName) {
                success = true;
                return res.json({ success, comment, commentorName });
            }

            return res.status(500).send("Unable to create a comment on the Community Post");
        } catch (error) {
            console.error(error);
...
router.post('/createCommentReply/:commentId', checkComment, [
    ...
    try {
        ...
        return res.status(500).send("Unable to reply a comment on the Community Post");
...
//like a comment if not previously liked
//localhost:3000/api/Community/getOne/653a2482efe81ef88c2b42a3/Post/653bc21bac5b8c1a425f09e8/likeComment/653c05bb07ae5fb5a38cc003
...
router.delete('/deleteComment/:commentId', checkComment, async (req, res) => {
    ...
    try {
        ...
        if (!comment) {
            const creatorCommunity = await Community.findById(communityId);

            if (creatorCommunity.creator_user_id === commentorId) {
        ...
        return res.status(500).send("Unable to delete the Comment");
    } catch (error) {
        console.error(error);
        return res.status(500).send("Some error occurred while processing the request");
    }
